<%
    ...
    println '<'+'%@ page import="iq.auth.*" %'+'>'
    print "<"
	println "%"
    println """
    def navigationGroupTemplate="{title: '_GROUP_', border:false, html: \\\"_ITEMS_\\\",icon: '_ICON_'}"
    def navigationItemTemplate="<a id='_CONTROLLER__ACTION_' href='#'><center><img src='/${applicationName}/images/_IMAGE_'/><br>_CHN_</center></a>"
        ...
        for(weight in weightsOfGroup)
        {
            ...
            for(domain in groupOfDomains)
            {
                if(group==domain.clazz.iqNavigation.group && weight==domain.clazz.iqNavigation.weight)
                {
                    ...
                    action="index"
                    print "    "*3
                    print "[controller: '$name', action:'Index', chn: '$chn', image: 'group.png']"
...
<%
            navigatableDomains.each{domain->
                    ...
                    println "        if(Ext.get('${domainName}Index')!=null){Ext.get('${domainName}Index').on('click', function(){ addTab('${domainName}','index','${domainChn}管理');});}"
            }
%>
});
